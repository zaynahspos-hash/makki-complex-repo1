...
const Rent = () => {
  ...
  const sendWhatsApp = (record: RentRecord) => {
    ...
    const previousRecords = rentRecords.filter(r => r.shopId === record.shopId && r.month < record.month);
    ...
    // 5. Construct Message
    const text = `
*${settings.plazaName.toUpperCase()} - RENT BILL*
--------------------------------
...
Target Rent: Rs. ${currentDue.toLocaleString()}
