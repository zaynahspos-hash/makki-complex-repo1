...
const Maintenance = () => {
  const { maintenanceCollections, shops, addMaintenancePayment, generateMonthlyMaintenance, createMaintenanceBill, settings, showToast } = useApp();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4 backdrop-blur-sm">
           <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-sm p-6 relative flex flex-col max-h-[90vh]">
              ...
              <div className="space-y-4 overflow-y-auto">
                 ...
                 <div className="flex flex-col gap-2">
                    ...
                    <div className="border border-gray-200 dark:border-gray-600 rounded-lg overflow-hidden flex flex-col">
                       ...
                       <div className="max-h-40 overflow-y-auto p-1 bg-white dark:bg-gray-800">
                          <div className={`px-3 py-2 text-sm rounded-md cursor-pointer flex justify-between ${exportSelectedShops.includes('all') ? 'bg-purple-50 text-purple-700 font-bold' : 'hover:bg-gray-50'}`} onClick={() => toggleShopSelection('all')}>
                             <span>All Shops</span>
                             {exportSelectedShops.includes('all') && <Check size={14}/>}
                          </div>
                          {filteredExportShops.map(s => (
                             <div key={s.id} className={`px-3 py-2 text-sm rounded-md cursor-pointer flex justify-between ${exportSelectedShops.includes(s.id) ? 'bg-purple-50 text-purple-700 font-bold' : 'hover:bg-gray-50'}`} onClick={() => toggleShopSelection(s.id)}>
                 ...
                 <button onClick={handleExport} className="w-full bg-purple-600 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 mt-2 shadow-lg hover:bg-purple-700">Download CSV</button>
              </div>
           </div>
        </div>
      )}
